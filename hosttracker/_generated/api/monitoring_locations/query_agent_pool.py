...
from ...models.agent_pool_query_request import AgentPoolQueryRequest
...
def _get_kwargs(
    *,
    body: AgentPoolQueryRequest | Unset = UNSET,
) -> dict[str, Any]:
...
def sync_detailed(
    *,
    ...
    """Query the same collection as listAgentPool, with the parameters in a JSON body.

     Takes exactly the parameters listAgentPool accepts on the query string, as one JSON object: a list-
    ...
    identical to what listAgentPool returns for the same values, and the caller needs the same scope -
    ...
    ids, free text carrying reserved characters - and stay on listAgentPool otherwise, since a GET can
    be cached and a POST cannot. A member this operation does not define is refused, exactly as an
    unknown query parameter is.

    Args:
        body (AgentPoolQueryRequest | Unset): The parameters, as one JSON object. A list-valued
    ...
    Returns:
        Response[AgentPoolPage | InternalError | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnknownField | UnknownParameter | ValidationFailed | UnsupportedMediaType]
    """
...
def sync(
    *,
    ...
    Returns:
        AgentPoolPage | InternalError | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnknownField | UnknownParameter | ValidationFailed | UnsupportedMediaType
    """
