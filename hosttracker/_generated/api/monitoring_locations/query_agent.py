...
def _get_kwargs(
    *,
    body: AgentQueryRequest | Unset = UNSET,
) -> dict[str, Any]:
...
def sync_detailed(
    *,
    ...
    """Query the same collection as listAgent, with the parameters in a JSON body.

     Takes exactly the parameters listAgent accepts on the query string, as one JSON object: a list-
    ...
    identical to what listAgent returns for the same values, and the caller needs the same scope - this
    is a read that happens to use POST, so it starts no job, writes nothing and takes no idempotency
    key. Reach for it when the filter is too long or too awkward for a url - a few hundred ids, free
    text carrying reserved characters - and stay on listAgent otherwise, since a GET can be cached and a
    POST cannot. A member this operation does not define is refused, exactly as an unknown query
    parameter is.

    Args:
        body (AgentQueryRequest | Unset): The parameters, as one JSON object. A list-valued filter
    ...
    Returns:
        Response[AgentPage | InternalError | InvalidCursor | InvalidLimit | UnknownEnumValue | UnknownField | UnknownParameter | ValidationFailed | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnsupportedMediaType]
    """
...
def sync(
    *,
    ...
    Returns:
        AgentPage | InternalError | InvalidCursor | InvalidLimit | UnknownEnumValue | UnknownField | UnknownParameter | ValidationFailed | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnsupportedMediaType
    """
