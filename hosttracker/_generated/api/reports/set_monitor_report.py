...
from ...models.report_subscription_set_request import ReportSubscriptionSetRequest
...
def _get_kwargs(
    ...
    contact_id: UUID,
    *,
    body: ReportSubscriptionSetRequest,
    ...
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 422:
        ...
        def _parse_response_422(
            data: object,
        ) -> UnknownEnumValue | UnknownParameter | UnsupportedReportChannel | ValidationFailed:
            try:
                if not isinstance(data, dict):
                    raise TypeError()
...
def sync_detailed(
    ...
    *,
    ...
    """Set the report-frequency set for this monitor-and-contact pair.
     ...
     Idempotently sets which report frequencies this contact receives for this monitor. `frequencies` is
    the EXACT desired set (at least one); use DELETE to remove the subscription. Reports are Email-only
    - a non-Email contact is refused. Answers with the resulting subscription. The contact's address is
    included only when the token also carries a contact read scope - a monitor-scoped token sees which
    contact it is, not how to reach it.

    Args:
        monitor_id (UUID):
        contact_id (UUID):
        idempotency_key (str | Unset):
        body (ReportSubscriptionSetRequest): The member vocabulary is closed: a member not listed
    ...
    Returns:
        Response[IdempotencyKeyConflict | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | MonitorReportSubscriptionView | NotFound | PayloadTooLarge | QuotaExceeded | UnknownEnumValue | UnknownParameter | UnsupportedReportChannel | ValidationFailed | UnsupportedMediaType]
    """
...
def sync(
    ...
    *,
    ...
    Returns:
        IdempotencyKeyConflict | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | MonitorReportSubscriptionView | NotFound | PayloadTooLarge | QuotaExceeded | UnknownEnumValue | UnknownParameter | UnsupportedReportChannel | ValidationFailed | UnsupportedMediaType
    """
