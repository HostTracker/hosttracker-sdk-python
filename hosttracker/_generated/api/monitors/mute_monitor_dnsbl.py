...
from ...models.dnsbl_mute_body import DnsblMuteBody
...
def _get_kwargs(
    monitor_id: UUID,
    *,
    body: DnsblMuteBody | Unset = UNSET,
    ...
    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/monitor/{monitor_id}/attached/dnsbl/mute".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = MonitorAttachedView.from_dict(response.json())
...
def sync_detailed(
    ...
    *,
    ...
    """Mute or unmute named blacklist zones on a monitor's blacklist check.
     ...
     Adds or removes mute flags on specific blacklist zones for one monitor and answers with the
    monitor's updated blacklist block, so the effect is visible without a second read. Use it to silence
    a listing you have accepted rather than disabling the whole blacklist check. Muting is convergent,
    so an Idempotency-Key is honoured but never required.

    Args:
        monitor_id (UUID):
        idempotency_key (str | Unset):
        body (DnsblMuteBody | Unset): The member vocabulary is closed: a member not listed here is
            refused rather than ignored. Every member is optional: what the body omits is left exactly
            as it was.
    ...
    Returns:
        Response[IdempotencyKeyConflict | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | MonitorAttachedView | NotFound | PayloadTooLarge | QuotaExceeded | TooManyItems | UnknownEnumValue | UnknownParameter | ValidationFailed | UnsupportedMediaType]
    """
...
def sync(
    ...
    *,
    ...
    Returns:
        IdempotencyKeyConflict | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | MonitorAttachedView | NotFound | PayloadTooLarge | QuotaExceeded | TooManyItems | UnknownEnumValue | UnknownParameter | ValidationFailed | UnsupportedMediaType
    """
