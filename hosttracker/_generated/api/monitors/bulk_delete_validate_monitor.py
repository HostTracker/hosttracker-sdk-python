...
from ...models.monitor_bulk_delete_validate_request import MonitorBulkDeleteValidateRequest
...
def _get_kwargs(
    *,
    body: MonitorBulkDeleteValidateRequest,
) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/monitor/bulk-delete-validate",
    }
...
def sync_detailed(
    *,
    ...
    """Check which monitors a delete filter selects, without deleting anything.
     ...
     Resolves a filter and answers how many monitors it selects together with a sample of them,
    identified by name, url and type. Nothing is written and no selection token is minted. It is the
    verification step of a bulk delete: run it, show a human what is about to go, then send the same
    filter to the delete itself. The answer is a snapshot - monitors created or removed between the two
    calls change what the delete resolves, which is why the delete reports its own count as well.

    Args:
        body (MonitorBulkDeleteValidateRequest): The member vocabulary is closed: a member not
    ...
    Returns:
        Response[FilterRequired | UnknownEnumValue | UnknownParameter | ValidationFailed | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | MonitorBulkDeleteValidateView | PayloadTooLarge | QuotaExceeded | UnsupportedMediaType]
    """
...
def sync(
    *,
    ...
    Returns:
        FilterRequired | UnknownEnumValue | UnknownParameter | ValidationFailed | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | MonitorBulkDeleteValidateView | PayloadTooLarge | QuotaExceeded | UnsupportedMediaType
    """
