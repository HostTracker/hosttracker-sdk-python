...
from ...models.monitor_bulk_delete_request import MonitorBulkDeleteRequest
from ...models.payload_too_large import PayloadTooLarge
from ...models.quota_exceeded import QuotaExceeded
from ...models.selection_mismatch import SelectionMismatch
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 409:
        ...
        def _parse_response_409(data: object) -> IdempotencyKeyConflict | SelectionMismatch:
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                response_409_type_0 = SelectionMismatch.from_dict(data)
...
def sync_detailed(
    *,
    ...
    """Delete every monitor a filter selects, as an asynchronous job.
     ...
     Resolves the filter to a concrete set of monitors, answers with a job id and the number accepted,
    and deletes them one at a time - each in its own transaction, each with its own receipt of what was
    cascaded away. The set is fixed at submission: a monitor that starts matching the filter afterwards
    is not touched. Check what a filter selects first with bulk-delete-validate, then send that filter
    here together with the count it reported as expectedCount: the server re-resolves and refuses if the
    number moved, so a delete can never be wider than the caller was shown. A filter that narrows by
    nothing is refused rather than taken to mean the whole account, and a selection larger than one
    operation carries is refused rather than trimmed. An Idempotency-Key is mandatory: this call answers
    202 and then works asynchronously, so a retry after a timeout would otherwise start a second job
    over the same targets. The 202 carries a Retry-After header saying how long to wait before the first
    poll, sized on how much work was accepted.

    Args:
        idempotency_key (str):
        body (MonitorBulkDeleteRequest): The member vocabulary is closed: a member not listed here
    ...
    Returns:
        Response[FilterRequired | TooManyItems | UnknownEnumValue | UnknownParameter | ValidationFailed | IdempotencyKeyConflict | SelectionMismatch | IdempotencyKeyRequired | MalformedRequest | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | JobAcceptedView | MethodNotAllowed | PayloadTooLarge | QuotaExceeded | UnsupportedMediaType]
    """
...
def sync(
    *,
    ...
    Returns:
        FilterRequired | TooManyItems | UnknownEnumValue | UnknownParameter | ValidationFailed | IdempotencyKeyConflict | SelectionMismatch | IdempotencyKeyRequired | MalformedRequest | InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidToken | JobAcceptedView | MethodNotAllowed | PayloadTooLarge | QuotaExceeded | UnsupportedMediaType
    """
