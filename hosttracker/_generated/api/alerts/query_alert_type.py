...
from ...models.alert_type_query_request import AlertTypeQueryRequest
...
def _get_kwargs(
    *,
    body: AlertTypeQueryRequest | Unset = UNSET,
) -> dict[str, Any]:
...
def sync_detailed(
    *,
    ...
    """Query the same collection as listAlertType, with the parameters in a JSON body.

     Takes exactly the parameters listAlertType accepts on the query string, as one JSON object: a list-
    ...
    identical to what listAlertType returns for the same values, and the caller needs the same scope -
    ...
    ids, free text carrying reserved characters - and stay on listAlertType otherwise, since a GET can
    be cached and a POST cannot. A member this operation does not define is refused, exactly as an
    unknown query parameter is.

    Args:
        body (AlertTypeQueryRequest | Unset): The parameters, as one JSON object. A list-valued
    ...
    Returns:
        Response[AlertTypeRowPage | InternalError | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnknownParameter | ValidationFailed | UnsupportedMediaType]
    """
...
def sync(
    *,
    ...
    Returns:
        AlertTypeRowPage | InternalError | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnknownParameter | ValidationFailed | UnsupportedMediaType
    """
