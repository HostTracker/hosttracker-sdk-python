...
from ...models.status_page_component_set_request import StatusPageComponentSetRequest
...
def _get_kwargs(
    id: UUID,
    *,
    body: StatusPageComponentSetRequest,
    ...
    _kwargs: dict[str, Any] = {
        "method": "put",
        "url": "/statuspage/{id}/component".format(
...
def _parse_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = StatusPageView.from_dict(response.json())
...
def sync_detailed(
    ...
    *,
    ...
    """Replace a status page's whole component set.
     ...
     A snapshot write, not a diff: send every component to keep, in display order - each either monitored
    (its monitor must be the account's own) or third-party (own name, optional manual state). Carry each
    existing row's id so per-component subscriptions survive the save. How many components a plan allows
    is enforced; a page already over its cap may re-save and reorder, only growing past it is refused.

    Args:
        id (UUID):
        idempotency_key (str | Unset):
        body (StatusPageComponentSetRequest): The member vocabulary is closed: a member not listed
    ...
    Returns:
        Response[IdempotencyKeyConflict | InsufficientRights | IpNotAllowed | MissingScope | PackageLimit | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | NotFound | PayloadTooLarge | QuotaExceeded | StatusPageView | UnknownEnumValue | UnknownParameter | ValidationFailed | UnsupportedMediaType]
    """
...
def sync(
    ...
    *,
    ...
    Returns:
        IdempotencyKeyConflict | InsufficientRights | IpNotAllowed | MissingScope | PackageLimit | InternalError | InvalidToken | MalformedRequest | MethodNotAllowed | NotFound | PayloadTooLarge | QuotaExceeded | StatusPageView | UnknownEnumValue | UnknownParameter | ValidationFailed | UnsupportedMediaType
    """
