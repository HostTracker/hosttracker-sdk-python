...
from ...models.contact_bulk_delete_request import ContactBulkDeleteRequest
...
def sync_detailed(
    *,
    ...
    """Delete every contact a filter selects, as an asynchronous job.
     ...
     Resolves the filter to a concrete set of contacts, answers with a job id and the number of contacts
    accepted, and deletes them one at a time - each in its own transaction, each with its own receipt of
    what was cascaded away. The set is fixed at submission: a contact that starts matching the filter
    afterwards is not touched. Check what a filter selects first with bulk-delete-validate; this call
    then acts on it, with no token to echo back. A filter that narrows by nothing is refused rather than
    taken to mean the whole account, and a selection larger than one operation carries is refused rather
    than trimmed. An Idempotency-Key is mandatory: this call answers 202 and then works asynchronously,
    so a retry after a timeout would otherwise start a second job over the same targets. The 202 carries
    a Retry-After header saying how long to wait before the first poll, sized on how much work was
    accepted.

    Args:
        idempotency_key (str):
        body (ContactBulkDeleteRequest): The member vocabulary is closed: a member not listed here
