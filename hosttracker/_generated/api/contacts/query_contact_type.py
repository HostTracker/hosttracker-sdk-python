...
from ...models.contact_type_query_request import ContactTypeQueryRequest
...
def _get_kwargs(
    *,
    body: ContactTypeQueryRequest | Unset = UNSET,
) -> dict[str, Any]:
...
def sync_detailed(
    *,
    ...
    """Query the same collection as listContactType, with the parameters in a JSON body.

     Takes exactly the parameters listContactType accepts on the query string, as one JSON object: a
    ...
    identical to what listContactType returns for the same values, and the caller needs the same scope -
    ...
    ids, free text carrying reserved characters - and stay on listContactType otherwise, since a GET can
    be cached and a POST cannot. A member this operation does not define is refused, exactly as an
    unknown query parameter is.

    Args:
        body (ContactTypeQueryRequest | Unset): The parameters, as one JSON object. A list-valued
    ...
    Returns:
        Response[ContactTypeRowPage | InternalError | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnknownParameter | ValidationFailed | UnsupportedMediaType]
    """
...
def sync(
    *,
    ...
    Returns:
        ContactTypeRowPage | InternalError | MalformedRequest | MethodNotAllowed | PayloadTooLarge | UnknownParameter | ValidationFailed | UnsupportedMediaType
    """
