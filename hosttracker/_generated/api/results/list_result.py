...
from ...models.list_result_expand_item import ListResultExpandItem
from ...models.list_result_fields_item import ListResultFieldsItem
from ...models.list_result_sort import ListResultSort
from ...models.list_result_state_item import ListResultStateItem
...
def _get_kwargs(
    *,
    ...
    state: list[ListResultStateItem] | Unset = UNSET,
    sort: ListResultSort | Unset = UNSET,
    expand: list[ListResultExpandItem] | Unset = UNSET,
    limit: int | Unset = 50,
    cursor: str | Unset = UNSET,
    fields: list[ListResultFieldsItem] | Unset = UNSET,
) -> dict[str, Any]:
...
def sync_detailed(
    *,
    ...
    """List raw check results across one or more monitors.
     ...
     Returns a page of individual check results, newest first, narrowed by time window, monitoring
    location, outcome (state=up|down) and by monitor. The monitor filters combine: monitor= (ids), url=
    (addresses, with like=true for substring match) and q= (free text over name and address) all narrow
    the same set, exactly as they do on the monitor list, and sending none reads the whole account's
    feed. Because an unfiltered read is the whole account, its time window is capped: an omitted from=
    takes that window and a wider explicit one is refused rather than quietly clipped - name the
    monitors with monitor= to read further back. Order it with sort=monitor to group the page by monitor
    (name A to Z, newest-first inside each) instead of the default sort=time. The summary endpoint
    answers aggregate uptime questions far more cheaply. Nothing is embedded by default: ask for
    ...
    inside it. Ask for expand=metrics to decode the check's stored measurements and, from the same
    document, the assertion rules it failed and the policy codes it violated.
    ...
    Args:
        ...
        state (list[ListResultStateItem] | Unset):
        sort (ListResultSort | Unset):
        expand (list[ListResultExpandItem] | Unset):
        limit (int | Unset):  Default: 50.
        cursor (str | Unset):
        fields (list[ListResultFieldsItem] | Unset):
    ...
    Returns:
        Response[InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidCursor | InvalidLimit | InvalidRange | TooManyItems | UnknownEnumValue | UnknownExpand | UnknownField | UnknownParameter | ValidationFailed | InvalidToken | MethodNotAllowed | QuotaExceeded | ResultPage | ServiceUnavailable]
    """
...
def sync(
    *,
    ...
    Returns:
        InsufficientRights | IpNotAllowed | MissingScope | InternalError | InvalidCursor | InvalidLimit | InvalidRange | TooManyItems | UnknownEnumValue | UnknownExpand | UnknownField | UnknownParameter | ValidationFailed | InvalidToken | MethodNotAllowed | QuotaExceeded | ResultPage | ServiceUnavailable
    """
