...
T = TypeVar("T", bound="MonitorGroupedAlertSubscriptionView")


@_attrs_define
class MonitorGroupedAlertSubscriptionView:
    """One element of the BY-MONITOR grouping of the account-wide alert list (`GET /alert/by-monitor`): one monitor and
    every contact subscribed to it, so the monitor's `{id,name,url,type}` is carried once per monitor rather than once
    per pair.
    ...
    subscriptions: list[MonitorAlertSubscriptionView] | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        if _subscriptions is not UNSET:
            ...
            for subscriptions_item_data in _subscriptions:
                subscriptions_item = MonitorAlertSubscriptionView.from_dict(subscriptions_item_data)

                subscriptions.append(subscriptions_item)

        monitor_grouped_alert_subscription_view = cls(
            monitor=monitor,
            subscriptions=subscriptions,
        )

        monitor_grouped_alert_subscription_view.additional_properties = d
        return monitor_grouped_alert_subscription_view
