...
from ..models.monitor_bulk_item_on_overlimit import MonitorBulkItemOnOverlimit, check_monitor_bulk_item_on_overlimit
from ..models.monitor_bulk_item_type import MonitorBulkItemType, check_monitor_bulk_item_type
...
T = TypeVar("T", bound="MonitorBulkItem")


@_attrs_define
class MonitorBulkItem:
    """One monitor to create, in the same shape POST /monitor takes. A member this entry omits is taken from `defaults`;
    the two are merged before the entry is validated, so an entry may look incomplete on its own and still be valid.
    ...
    on_overlimit: MonitorBulkItemOnOverlimit | Unset = UNSET
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        on_overlimit: MonitorBulkItemOnOverlimit | Unset
        if isinstance(_on_overlimit, Unset):
            on_overlimit = UNSET
        else:
            on_overlimit = check_monitor_bulk_item_on_overlimit(_on_overlimit)
        ...
        type_: MonitorBulkItemType | Unset
        if isinstance(_type_, Unset):
            type_ = UNSET
        else:
            type_ = check_monitor_bulk_item_type(_type_)

        url = d.pop("url", UNSET)

        monitor_bulk_item = cls(
            add_tags=add_tags,
