...
T = TypeVar("T", bound="ContactView")


@_attrs_define
class ContactView:
    """**The v2 contact resource**. **`sendCost` is on the wire; `sendCostDirty` is not.** The price of one message is
    something a customer is entitled to read (it is what the first-party contacts dashboard shows beside a paid
    channel), so it is published for the types that HAVE one and omitted for the rest. The dirty flag beside it in
    storage is re-pricing bookkeeping and stays internal. **`alertDelay` is MINUTES**, never the stored `DownAlertLevel`
    index. A row whose stored index is off the frozen ladder - which the engine drops with a warning - reads back `null`
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        contact_view.additional_properties = d
        return contact_view
