...
if TYPE_CHECKING:
    ...
    from ..models.inline_contact_result_view import InlineContactResultView
...
T = TypeVar("T", bound="MonitorWriteResult")


@_attrs_define
class MonitorWriteResult:
    """The monitor, in exactly the shape `GET /monitor/{id}` renders it, plus the inline-contact results when the request
    carried a `contacts` block.
    ...
    """
    contacts: list[InlineContactResultView] | Unset = UNSET
    """ One entry per inline contact the request declared, in the same order - the caller's join column back to its
    own `ref`. Absent when the request declared none. """
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        if _contacts is not UNSET:
            ...
            for contacts_item_data in _contacts:
                contacts_item = InlineContactResultView.from_dict(contacts_item_data)

                contacts.append(contacts_item)

        monitor_write_result = cls(
            id=id,
            since=since,
            enabled=enabled,
            updated=updated,
        ...
        monitor_write_result.additional_properties = d
        return monitor_write_result
